...
buildPPDF <- function(performanceDF, preferDF){
    ...
    for (s in 1:max(performanceDF$Scenario)){
        workDF <- workDF %>% filter(performanceDF$Scenario == s) %>%
            filter(Time =< max(preferDF$Time)) %>%
                       bind_cols(workDF, preferDF)
...
buildPref <- function(preferList){
    for (t in 1:nrow(preferList)){
